...
public class ConstrainedBlockIterator implements BlockIterator {
	...
	public LatticeBlock next() {
		...
		return new ConstrainedBlock(sitePos, siteData, f, offs, zSize,
				yzSize, xyzSize);
	}
	...
	private void extractValuesForCurrentSequence(double[] siteData,
		...
		for(int q = 0; q < 19; ++q) {
			...
			int toExtractFrom = arrayStart + (bOff + currentX*yzSize + currentY*zSize + currentZ)%xyzSize;
			int toExtractTo = arrayStart + (bOff + currentX*yzSize + currentY*zSize + zStop)%xyzSize;

			// Values are sequentially extracted from f but need to
			// be rearranged in siteData.
			if(toExtractFrom <= toExtractTo) {
				...
				// A warp is needed
				int off = extractedSitesForBlock * 19 + q;
				for(int pos = toExtractFrom; pos < arrayStop; ++pos) {
				...
				for(int pos = arrayStart; pos < toExtractTo; ++pos) {
